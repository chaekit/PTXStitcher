...
def compilation_path1(base_file_name):
    ...
    front_end_stage = "clang -Dcl_clang_storage_class_specifiers -isystem /home/hskim/work/libclc/install/include -include clc/clc.h -target nvptx64-- {0} -emit-llvm -S -o {1}".format(cl_file_name, ir_file_name)
    ...
    linker_stage = "llvm-link /home/hskim/work/libclc/install/lib/clc/nvptx64--nvidiacl.bc {0} -o {1}".format(ir_file_name, linked_file_name)
...
cl_file_name = "%s.cl" % base_file_name
ir_file_name = "%s.ll" % base_file_name
front_end_stage = "clang -Dcl_clang_storage_class_specifiers -I/home/hskim/work/libclc/include/generic -I/home/hskim/work/libclc/include/ptx -include clc/clc.h -target nvptx64--nvidiacl {0} -emit-llvm -S -o {1}".format(cl_file_name, ir_file_name)
subprocess.call(front_end_stage.split())
...
ptx_file_name = "%s.nvptx.s" % base_file_name
backend_stage = "clang -target nvptx64--nvidiacl {0} -S -o {1}".format(linked_file_name, ptx_file_name)
subprocess.call(backend_stage.split())
