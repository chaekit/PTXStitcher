...
macro_flags = filter(lambda s: s[0:2] == "-D", sys.argv)
flags_without_macros = filter(lambda s: s[0:2] != "-D", sys.argv)
flattened_macro_flags = " ".join(macro_flags)

parser.add_option("-D", type="string", dest="macro_flags", default=flattened_macro_flags)
(options, args) = parser.parse_args(flags_without_macros)

base_file_name = base_file_name(flags_without_macros[-1])
